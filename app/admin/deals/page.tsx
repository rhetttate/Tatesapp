...
export default function AdminDealsPage() {
  ...
  const [filter, setFilter] = useState<
    "" | "featured" | "active" | "meat" | "produce" | "grocery" | "new" | "weekly"
  >("featured");
  ...
  async function loadDeals() {
    setLoading(true);
    // NOTE: don't wipe status here; keep last message unless we actually error
    const { data, error } = await supabase
      .from("deals")
      .select(
        "id,name,price,upc,description,image_url,active,department,featured,featured_order,start_date,end_date,created_at"
      )
  ...
  async function addDeal() {
    ...
    try {
      ...
      const fo = Number(digitsOnly(featuredOrder) || "0");
      ...
      if (imageFile) {
        try {
          ...
          throw new Error("Image upload failed: " + (e?.message ?? String(e)));
        }
      }
      ...
      // IMPORTANT: select the inserted row so we can update UI without reloading
      const { data, error } = await supabase
        .from("deals")
        .insert(payload)
        .select(
                ...
                <div style={{ flex: "1 1 280px", minWidth: 260 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "baseline" }}>
                    <input
                      ...
                      value={d.name}
                      onChange={(e) =>
                        setDeals((prev) => prev.map((x) => (x.id === d.id ? { ...x, name: e.target.value } : x)))
                      }
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 10, marginTop: 10 }}>
                    ...
                    <div style={{ gridColumn: "span 6" }}>
                      ...
                      <input
                        ...
                        value={d.upc}
                        onChange={(e) =>
                          setDeals((prev) => prev.map((x) => (x.id === d.id ? { ...x, upc: e.target.value } : x)))
                        }
                        onBlur={(e) => {
                          try {
                            const u = normalizeUpc(e.target.value);
                            updateDeal(d.id, { upc: u });
                          } catch {
                            // ignore invalid on blur
                          }
                        }}
                      />
                    </div>
                    ...
                    <div style={{ gridColumn: "span 12" }}>
                      ...
                      <textarea
                        ...
                        value={d.description || ""}
                        onChange={(e) =>
                          setDeals((prev) => prev.map((x) => (x.id === d.id ? { ...x, description: e.target.value } : x)))
                        }
                    ...
                    <div style={{ gridColumn: "span 12" }}>
                      ...
                      <input
                        ...
                        value={d.image_url || ""}
                        onChange={(e) =>
                          setDeals((prev) => prev.map((x) => (x.id === d.id ? { ...x, image_url: e.target.value } : x)))
                        }
                    ...
                    <div style={{ gridColumn: "span 12", display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <input
                          ...
                          inputMode="numeric"
                          onChange={(e) =>
                            setDeals((prev) => prev.map((x) => (x.id === d.id ? { ...x, featured_order: Number(e.target.value) } : x)))
                          }
                          onBlur={(e) => updateDeal(d.id, { featured_order: Number(digitsOnly(e.target.value) || "0") })}
                        />
                      </div>
                    </div>
